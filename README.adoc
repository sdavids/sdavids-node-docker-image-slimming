...
====
* use https://webpack.js.org[webpack] to bundle the example REST API
+
[NOTE]
====
...
$ docker run --rm de.sdavids/sdavids-node-docker-image-slimming:local stat -c "%s" /node/server.cjs
1189
...
16.4kB    COPY --chown=node:node /opt/app/dist/server.…
